...
public:
    CodeGenerator(LLVMContext &context, IRBuilder<> &builder, Module &module,
                  std::map<std::string, AllocaInst *> *namedValues, std::map<std::string, PrototypeAST> *FunctionProtos)
    ...
    void Visit(FunctionAST &ast) override {
        //(*FunctionProtos)[ast.getProto().getName()] = ast.getProto();
